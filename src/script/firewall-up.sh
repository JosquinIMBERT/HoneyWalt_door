...
#iptables -A PREROUTING -t mangle -p tcp --dport 22 # Accept attackers connections
#iptables -A PREROUTING -t mangle -p udp -i wg-srv --sport 1024:65535 --dport 53 -m state --state NEW,ESTABLISHED -j ACCEPT # Accept DNS requests
#iptables -A PREROUTING -t mangle -p udp --sport 53 --dport 1024:65535 -m state --state ESTABLISHED -j ACCEPT # Accept the answers to DNS requests
